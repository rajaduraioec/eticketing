...
class Reportdaywise extends BE_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Reportdaywise_model');
        $this->ctrl_name="reportdaywise";
        $this->module_name="daywise";
    }
    public function index() {   
        if($this->rauth->has_access($this->module_name))
        {
            if($this->rauth->is_depot()):
                ...
                $data['depot']=NULL;
                $data['depots']=$this->Reportdaywise_model->_getdepots();
            endif;
    ...
    public function ajax_response(){
        ...
        $data['depot']      = $depot;
        $date=explode('-', $range);
        $data['results']      = $this->Reportdaywise_model->_getdata($date,$depot);
        $data['records']      = array();
        $data['ictdata']=$this->ictdata;
        if($this->rauth->is_depot()):
            $data['depotck']=1;
