 ...
 * @filesource  Reportdaywise.controllers
 */

class Reporttripwise extends BE_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Reporttripwise_model');
        $this->ctrl_name="reporttripwise";
        $this->module_name="tripwise";
    }
    public function index() {   
        if($this->rauth->has_access($this->module_name))
        {
            if($this->rauth->is_depot()):
                ...
                $data['depot']=NULL;
                $data['depots']=$this->Reporttripwise_model->_getdepots();
            endif;
    ...
    public function ajax_response(){
        ...
        $data['depot']      = $depot;
        $date=explode('-', $range);
        $data['results']      = $this->Reporttripwise_model->_getdata($date,$depot);
        $data['records']      = array();
        $data['ictdata']=$this->ictdata;
        if($this->rauth->is_depot()):
            $data['depotck']=1;
