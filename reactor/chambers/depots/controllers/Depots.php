 ...
 * @since       Version 1.0.0
 * @module      Depots
 * @filesource  depots.controllers
 */

class Depots extends BE_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Depots_model');
        $this->ctrl_name="depots";
        $this->module_name="depots";
    }
    public function index() {   
        if($this->rauth->has_access($this->module_name))
        {
            $data['page_name']="Depots";
    ...
    public function ajax_list(){
        echo json_encode($this->Depots_model->_getdata());
    }
    public function create(){
        if($this->rauth->has_accesslevel($this->module_name,'create'))
        {
            $status=$this->Depots_model->create();
    ...
    public function edit($id='') {
        $data['id']=$id;
        $data['ictdata']=$this->ictdata;
        $data['info']=$this->Depots_model->getinfo($id);
        $data['ctrl_name']=$this->ctrl_name;
        $this->rview->gen('edit',$data);
    }
    public function save($id) {
        $status=$this->Depots_model->edit($id);
        echo json_encode($this->rview->astatus($status,TRUE));
    }
    public function exists($value='',$id=''){
        if($this->input->is_ajax_request())
        {
            $check=$this->Depots_model->exists($value);
                if(count($check)>0){
                    if($id!=''){
