 ...
 * @since       Version 1.0.0
 * @module      Expenses
 * @filesource  Expenses.controllers
 */

class Expenses extends BE_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Expenses_model');
        $this->ctrl_name="expenses";
        $this->module_name="expenses";
    }
    ...
    public function ajax_list(){
        echo json_encode($this->Expenses_model->_getdata());
    }
    public function create(){
        if($this->rauth->has_accesslevel($this->module_name,'create'))
        {
            $status=$this->Expenses_model->create();
    ...
    public function edit($id='') {
        $data['id']=$id;
        $data['ictdata']=$this->ictdata;
        $data['info']=$this->Expenses_model->getinfo($id);
        $data['ctrl_name']=$this->ctrl_name;
        $this->rview->gen('edit',$data);
    }
    public function save($id) {
        $status=$this->Expenses_model->edit($id);
        echo json_encode($this->rview->astatus($status,TRUE));
    }
    public function exists($value='',$id=''){
        if($this->input->is_ajax_request())
        {
            $check=$this->Expenses_model->exists($value);
                if(count($check)>0){
                    if($id!=''){
                        if($check[0]['id_expenses']==$id){
                            $result['content'] = "";
                            $result['status']=TRUE;
                        }else{
