 ...
 * @since       Version 1.0.0
 * @module      Reportcardbased
 * @filesource  Reportcardbased.controllers
 */

class Reportcardbased extends BE_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Reportcardbased_model');
        $this->ctrl_name="reportcardbased";
        $this->module_name="card-based-report";
    }
    public function index() {   
        if($this->rauth->has_access($this->module_name))
        {
            ...
            if($this->rauth->is_depot()):
                ...
                $data['depot']=NULL;
                $data['depots']=$this->Reportcardbased_model->_getdepots();
            endif;
    ...
    public function ajax_response(){
        ...
        $date=explode('-', $range);
        $data['ictdata']=$this->ictdata;
        $data['results']      = $this->Reportcardbased_model->_getdata($date,$depot);
        $data['records']      = array();
        if($this->rauth->is_depot()):
            $data['depotck']=1;
        else:
