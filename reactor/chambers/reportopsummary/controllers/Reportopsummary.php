 ...
 * @filesource  Reportopsummary.controllers
 */

class Reportopsummary extends BE_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Reportopsummary_model');
        $this->ctrl_name="reportdepotwise";
        $this->module_name="op-summary-report";
    }
    public function index() {   
        if($this->rauth->has_access($this->module_name))
        {
            ...
            if($this->rauth->is_depot()):
                ...
                $data['depot']=NULL;
                $data['depots']=$this->Reportopsummary_model->_getdepots();
            endif;
    ...
    public function ajax_response(){
        ...
        $data['depot']      = $depot;
        $date=explode('-', $range);
        $data['results']      = $this->Reportopsummary_model->_getdata($date,$depot);
        $data['records']      = array();
        $data['ictdata']=$this->ictdata;
        if($this->rauth->is_depot()):
            $data['depotck']=1;
