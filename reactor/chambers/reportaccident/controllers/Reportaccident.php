 ...
 * @filesource  Reportaccident.controllers
 */

class Reportaccident extends BE_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Reportaccident_model');
        $this->ctrl_name="reportaccident";
        $this->module_name="accident-report";
    }
    public function index() {   
        if($this->rauth->has_access($this->module_name))
        {
            if($this->rauth->is_depot()):
                ...
                $data['depot']=NULL;
                $data['depots']=$this->Reportaccident_model->_getdepots();
            endif;
    ...
    public function ajax_response(){
        ...
        $data['depot']      = $depot;
        $date=explode('-', $range);
        $data['results']      = $this->Reportaccident_model->_getdata($date,$depot);
        $data['ictdata']=$this->ictdata;
        $data['records']      = array();
        if($this->rauth->is_depot()):
            $data['depotck']=1;
